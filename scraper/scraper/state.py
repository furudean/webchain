...
from scraper.crawl import CrawledNode
...
class StateTable:
    ...
    def insert(self, node_to_insert: CrawledNode):
        self.nodes.append(node_to_insert)
        return len(self.nodes) - 1

    def find(self, search_key):
        for i in range(0, len(self.nodes)):
            if self.nodes[i].at == search_key:
                return i
        return -1
    ...
    def log(self):
        nodes = []
        for i in range(0, len(self.nodes)):
            nodes.append(dataclasses.asdict(self.nodes[i]))
        r = {}
